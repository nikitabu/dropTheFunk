...
//passing a function within $() effectively acts as an onLoad() function

$(function() {
  var b, booleans, getScaledImageData, link, params, scApiUrl, source_origin, strtobool, timecode, _i, _len;
  
  // create Track model, by extending Backbone.Model
  ...
  window.TrackList = Backbone.Collection.extend({
		...
		//stop the current track and start the one that was just clicked on
  ...
  // create a new View for TrackView
  window.TrackView = Backbone.View.extend({
    ...
    render: function() {
      return $(this.el).html(ich.track(this.model.toJSON())); //ich is from icanhaz.sj
    },
  ...
  // create a new View for AppView
  window.AppView = Backbone.View.extend({
		el: $('#stratus'), //the DOM element representing the view
		...
		defaults: {
		  ...
		  download: true,
		  env: 'production',
		  key: '9e398c66c27f97715dd234796745b535', //stratus's key, should change this to our's when we move it to our server
		  links: ['https://soundcloud.com/majorlazer/major-lazer-workout-mix'],
		  random: false,
		...
		initialize: function() {
		  ...
		  that = this;
		  this.options = options = _.extend(this.defaults, this.options); // the _. is from underscore.js
		  Tracks.bind('add', this.add, this);  // attach events & handlers to Tracks
		  ...
		  console.log("Stratus initialized");
		  ...
		  return SC.whenStreamingReady(function() {
				return that.loadTracks(options.links, function() {
				  ...
				  $('head').append(ich.theme(options)); //ich is from icanhaz.js
				  if (SC.isConnected()) {
					that.updateUser();
				  }
		...
		render: function() {
		  ...
		  this.$('#player .track').html(ich.current(data)); //ich is from icanhaz.js
  ...
  //source_origin = 'player';
  
  //recieving a message from stratus.sc
  //defunct
  /*
  $.receiveMessage(function(e) {
    var result, url;
    url = e.data; //url = url of track
    result = Tracks.find(function(track) {			//searches through each track in Tracks
      return track.get('permalink_url') === url; 	//if the url corresponds to something in Tracks, returns true
    });
    if (result) {
      return Tracks.toggle(result);					//if true, play the track
    } else {
      return SC.get("/resolve", {					//if not already in Tracks, resolve the track with the given url and then add the result to Tracks
        url: url
      }, function(track) {
        Tracks.add(track);
  ...
  */
	...
	// add a track to Tracks
  $.addToTracks = function(track_url) {
    var result, url;
    url = track_url; //url = url of track
    result = Tracks.find(function(track) {			//searches through each track in Tracks
      return track.get('permalink_url') === url; 	//if the url corresponds to something in Tracks, returns true
    });
    if (result) {
      ...
      return SC.get("/resolve", {					//if not already in Tracks, resolve the track with the given url and then add the result to Tracks
        ...
        Tracks.add(track);							//add the track to the collection
        //return Tracks.play(Tracks.get(track.id)); //automatically play next track (comment out if you just want to add to playlist)
      });
    }
  };
  ...
  //loop through params and convert any 'true' strings to true if necessary
  ...
  console.log("player.js has been executed");
